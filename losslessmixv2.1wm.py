...
for key in tqdm(a.keys(), desc="Stage 1/3"):
    ...
    if "model" in key:
        values = [m.get(key, None) for m in models]
        if None in values:
            continue
        values = [v for v in values if v is not None]
        weights = [1.0 / len(values) for _ in values]
        avg_value = sum([w*v for w, v in zip(weights, values)])
        if step == 1 or step == 2:
            print(f'step:{step}')
        ...
        try:
            if args.maxdiff:
                ...
                weighted_values = [w*v for w, v in zip(weights, values)]
                weighted_avg_value = sum(weighted_values)
                a[key] = weighted_avg_value
